...
router.get("/adminProducts", isAdmin, async (req, res) => {
  try {
    ...
    res.render("adminProducts", { products, email: req.session.email, first_name: req.session.first_name, last_name: req.session.last_name, age: req.session.age });
  } catch (error) {
...
router.get("/products", async (req, res) => {
  try {
    ...
    res.render("products", { products, email: req.session.email, first_name: req.session.first_name, last_name: req.session.last_name, age: req.session.age });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
